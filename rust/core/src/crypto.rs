use anyhow::{anyhow, Context};
use chrono::{Duration, Utc};
use jsonwebtoken::{encode, Algorithm, EncodingKey, Header};
use serde::{Deserialize, Serialize};

use crate::KelivoResult;

#[derive(Debug, Clone)]
pub struct GoogleAuthJwtRequest {
    pub client_email: String,
    pub private_key_pem: String,
    pub token_uri: String,
    pub scopes: Vec<String>,
}

#[derive(Debug, Serialize, Deserialize)]
struct Claims {
    iss: String,
    scope: String,
    aud: String,
    iat: i64,
    exp: i64,
}

pub fn create_google_auth_jwt(request: GoogleAuthJwtRequest) -> KelivoResult<String> {
    create_google_auth_jwt_impl(request).map_err(Into::into)
}

fn create_google_auth_jwt_impl(request: GoogleAuthJwtRequest) -> anyhow::Result<String> {
    let email = request.client_email.trim();
    if email.is_empty() {
        return Err(anyhow!("client_email is required"));
    }

    let audience = request.token_uri.trim();
    if audience.is_empty() {
        return Err(anyhow!("token_uri is required"));
    }

    let scope = normalize_scopes(&request.scopes)?;

    let signing_key = EncodingKey::from_rsa_pem(request.private_key_pem.trim().as_bytes())
        .context("invalid RSA private key")?;

    let now = Utc::now();
    let issued_at = now.timestamp();
    let expires_at = (now + Duration::hours(1)).timestamp();

    let claims = Claims {
        iss: email.to_owned(),
        scope,
        aud: audience.to_owned(),
        iat: issued_at,
        exp: expires_at,
    };

    let mut header = Header::new(Algorithm::RS256);
    header.typ = Some("JWT".to_string());

    encode(&header, &claims, &signing_key).context("failed to encode JWT")
}

fn normalize_scopes(scopes: &[String]) -> anyhow::Result<String> {
    let values: Vec<String> = scopes
        .iter()
        .map(|scope| scope.trim())
        .filter(|scope| !scope.is_empty())
        .map(ToOwned::to_owned)
        .collect();

    if values.is_empty() {
        return Err(anyhow!("at least one scope is required"));
    }

    Ok(values.join(" "))
}

#[cfg(test)]
mod tests {
    use super::*;
    use jsonwebtoken::{decode, Algorithm, DecodingKey, Validation};

    fn sample_request() -> GoogleAuthJwtRequest {
        GoogleAuthJwtRequest {
            client_email: "test-service@example.iam.gserviceaccount.com".to_string(),
            private_key_pem: SAMPLE_PRIVATE_KEY.to_string(),
            token_uri: "https://oauth2.googleapis.com/token".to_string(),
            scopes: vec!["https://www.googleapis.com/auth/cloud-platform".into()],
        }
    }

    const SAMPLE_PRIVATE_KEY: &str = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n";

    #[test]
    fn creates_signed_jwt() {
        let token = create_google_auth_jwt(sample_request()).expect("token");

        let mut validation = Validation::new(Algorithm::RS256);
        validation.validate_aud = false;
        validation.validate_exp = false;
        validation.insecure_disable_signature_validation();
        let decoded = decode::<Claims>(&token, &DecodingKey::from_secret(&[]), &validation)
            .expect("decode token");
        assert_eq!(
            decoded.claims.iss,
            "test-service@example.iam.gserviceaccount.com"
        );
        assert_eq!(
            decoded.claims.scope,
            "https://www.googleapis.com/auth/cloud-platform"
        );
        assert_eq!(
            decoded.claims.aud,
            "https://oauth2.googleapis.com/token".to_string()
        );
        assert_eq!(decoded.claims.exp - decoded.claims.iat, 3600);
    }

    #[test]
    fn returns_error_for_invalid_key() {
        let mut request = sample_request();
        request.private_key_pem = "INVALID_KEY".to_string();

        let err = create_google_auth_jwt(request).expect_err("expected failure");
        assert!(err.message().contains("invalid RSA private key"));
    }

    #[test]
    fn requires_non_empty_scopes() {
        let mut request = sample_request();
        request.scopes = vec!["".into()];

        let err = create_google_auth_jwt(request).expect_err("expected failure");
        assert!(err.message().contains("at least one scope"));
    }

    #[test]
    fn requires_client_email() {
        let mut request = sample_request();
        request.client_email = " ".to_string();

        let err = create_google_auth_jwt(request).expect_err("expected failure");
        assert!(err.message().contains("client_email is required"));
    }

    #[test]
    fn requires_token_uri() {
        let mut request = sample_request();
        request.token_uri = "".to_string();

        let err = create_google_auth_jwt(request).expect_err("expected failure");
        assert!(err.message().contains("token_uri is required"));
    }
}
