...
impl GoogleAdapter {
    pub fn new(endpoint: impl Into<String>, api_key: impl Into<String>) -> KelivoResult<Self> {
        Ok(Self {
            inner: OpenAiAdapter::with_provider("google", endpoint, api_key)?,
        })
    }
    ...
    pub fn with_client(
        ...
        Self {
            inner: OpenAiAdapter::with_client_and_provider(client, "google", endpoint, api_key),
        }
    }
}

#[async_trait]
impl LlmProvider for GoogleAdapter {
    fn id(&self) -> &str {
        self.inner.id()
    }
